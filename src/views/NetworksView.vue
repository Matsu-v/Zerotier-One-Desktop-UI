        ...
        <template v-slot:expanded-row="{ columns, item }">
            <tr>
                <td class="pa-0" :colspan="columns.length">
                    ...
                    <v-card class="ml-0;pa-1"><v-card-title> More info about {{ item.id }}</v-card-title>
                        <v-card-text>
                            <v-row dense>
                                <v-col cols="12" sm="6">
                                    <v-checkbox @change="SettingsEdited = true" label="Allow DNS" v-model="item.allowDNS"
                                        dense></v-checkbox>
                                </v-col>
                                <v-col cols="12" sm="6">
                                ...
                                <v-col cols="12" sm="6">
                                    <v-text-field label="Bridge" v-model="item.bridge" readonly dense></v-text-field>
                                </v-col>
                                <v-col cols="6" sm="3">
                                ...
                                <v-col cols="12" sm="12">
                                    <v-text-field label="Routes" v-model="item.routes" readonly dense></v-text-field>
                                </v-col>
                            </v-row>
                        </v-card-text>
                        <v-card-actions>
                            <v-expand-transition>
                                <div v-if="SettingsEdited">
                                    <v-btn :loading="UpdatingSettings" text color="success" @click="SaveSettings(item)">Save
                                        settings</v-btn>
                                    <v-btn  text color="warning"
                                        @click="SettingsEdited = false; fetchStatus()">Discard changes</v-btn>
                                </div>
                            </v-expand-transition>
                        </v-card-actions>
                    </v-card>
